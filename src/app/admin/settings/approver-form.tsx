  ...
  return (
    <form action={formAction} className="space-y-4">
      ...
      <div className="space-y-1.5 max-w-xs">
        <Label>{t('adminSettings.defaultApprover')}</Label>
        <Select value={value} onValueChange={v => v && setValue(v)}>
          <SelectTrigger>
            <SelectValue placeholder={t('adminSettings.noDefaultApprover')} />
          </SelectTrigger>
          <SelectContent>
        ...
        <p className="text-xs text-gray-400">{t('adminSettings.noDefaultApproverDesc')}</p>
      </div>
        ...
        <form action={formAction} className="flex items-center gap-2">
          ...
          <Select value={value} onValueChange={v => v && setValue(v)}>
            <SelectTrigger className="h-8 text-sm w-48">
              <SelectValue placeholder={t('adminSettings.useDefault')} />
            </SelectTrigger>
            <SelectContent>
              <SelectItem value="__none__">{t('adminSettings.useDefault')}</SelectItem>
              {approvers.map(a => (
                <SelectItem key={a.id} value={a.id}>
                  {a.full_name ?? a.id}
                </SelectItem>
...
export default function ApproverForm({
  ...
  return (
    <div className="space-y-8">
      ...
      {brandRows.length > 0 && (
        <div>
          <p className="text-sm font-medium text-gray-700 mb-3">{t('adminSettings.brandApproverTitle')}</p>
          <p className="text-xs text-gray-400 mb-3">
            {t('adminSettings.brandApproverDesc')}
          </p>
          <div className="border border-gray-100 rounded-lg overflow-hidden">
            <table className="w-full">
              <thead className="bg-gray-50">
                <tr>
                  <th className="text-left px-4 py-2.5 text-xs font-medium text-gray-500">{t('projects.brand')}</th>
                  <th className="text-left px-4 py-2.5 text-xs font-medium text-gray-500">{t('adminSettings.assignedApprover')}</th>
                </tr>
              </thead>
              <tbody>
                {brandRows.map(row => (
