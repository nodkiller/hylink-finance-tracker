...
export default function ProjectApprovalButtons({ projectId, brandName }: Props) {
  ...
  const wrappedApprove = async (_prev: State, formData: FormData): Promise<State> => {
    ...
    if (result && 'success' in result && result.success) {
      ...
      toast(code ? t('projects.projectApprovedWithCode').replace('{code}', code) : t('projects.projectApprovedNoCode'), 'success')
      router.refresh()
    }
    return result
  }
  ...
  const wrappedReject = async (_prev: State, formData: FormData): Promise<State> => {
    ...
    if (result && 'success' in result && result.success) {
      setRejectOpen(false)
      toast(t('projects.projectRejectedMsg'), 'error')
      router.refresh()
    }
    return result
  }
    ...
    <>
      ...
      <Dialog open={approveOpen} onOpenChange={setApproveOpen}>
        <DialogContent className="max-w-sm">
          <DialogHeader>
            <DialogTitle>{t('projects.approveProjectTitle')}</DialogTitle>
          </DialogHeader>
          <form action={approveFormAction} className="space-y-4 pt-2">
            <p className="text-sm text-gray-500">
              {t('projects.approveDesc')}
            </p>
            <div className="space-y-1.5">
              <Label htmlFor="approve-comment">{t('projects.commentOptional')}</Label>
              <Input id="approve-comment" name="comment" placeholder={t('projects.commentPlaceholder')} />
            </div>
      ...
      <Dialog open={rejectOpen} onOpenChange={setRejectOpen}>
        <DialogContent className="max-w-sm">
          <DialogHeader>
            <DialogTitle>{t('projects.rejectProject')}</DialogTitle>
          </DialogHeader>
          <form action={rejectFormAction} className="space-y-4 pt-2">
            <p className="text-sm text-gray-500">
              {t('projects.rejectDesc')}
            </p>
            <div className="space-y-1.5">
              <Label htmlFor="reject-reason">{t('projects.rejectionReasonOptional').replace(/[（(].*[）)]/, '')} <span className="text-red-500">*</span></Label>
              <Input id="reject-reason" name="reason" placeholder={t('projects.explainRejection')} required />
            </div>
